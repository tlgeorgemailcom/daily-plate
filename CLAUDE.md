...
- **🚨 TODO — `fill_class_hint` column needed in `DataCentralCombo` for community recipe accuracy (July 2026).** `inferFillingClass()` in `src/lib/nutrition/inferFillingClass.ts` is entirely pastry-oriented — it has **zero rules for any meat protein**. Every community recipe section containing pork, beef, chicken, or fish that falls through `inferFillingClass()` gets `fill_class='none'` → `yfw=1.0` → no moisture loss at all. That is deeply wrong nutrition for any cooked protein. The fix requires all of the following changes in one pass:
  1. **`DataCentralCombo` new column `fill_class_hint TEXT`** — per-NDB recommended fill_class when the ingredient is the primary protein in an oven-baked or braised section. Add via `ALTER TABLE DataCentralCombo ADD COLUMN fill_class_hint TEXT` in local comboo.db and sync to Turso SR28. Populate immediately for calibrated NDBs: `10080 → 'baked_pork'`, `13803 → 'braised_beef'`, `5062 → 'pan_grilled_chicken'`. Add more NDBs as new fill classes are calibrated.
  2. **Food search API** — read `fill_class_hint` from `DataCentralCombo` and include it in the search result payload.
  3. **`NutrientRow` (`src/lib/nutrition/types.ts`)** — add `fillClassHint?: string` field.
  4. **`buildRecipeCommunityV3.ts`** — before calling `inferFillingClass()`, check if any ingredient in the section has a `fillClassHint` value AND the section's `cookMethod` is `baked` or `braise`. If so, use that hint as the `fillClass` instead of calling `inferFillingClass()`.
  5. **`load_comboo_nutrients()` in `load.py`** — read `fill_class_hint` from `DataCentralCombo` and store it as `nuts["_fill_class_hint"]` for potential Python-path use (future).
