...
**⛔ Component-ref sections MUST have `cook_method='raw'` in `recipe_sections.csv`** — `validate_ledger.py` enforces this as a hard error: `"component-ref @CHILD_ID so cook_method must be 'raw' (got 'X')"`). The display cook method (e.g. 'simmer', 'pan grilled') is supplied at upload time by `upload.py`'s `_component_cook_method()` which reads the child recipe's actual method. Never set the child's cook method directly in `recipe_sections.csv` — it will fail validation. Affected sections are any row with `source_recipe` set in column 15. Found and fixed across 6 sections (SAUCE_002-006, BKFST_003, BKFST_037) during July 2026 SAUCE unlock session.
...
**⚠️ When changing a section's `prep_method` from `''` to an active method during the unlock process, always set `cook_minutes` directly in `recipes.csv`.** `upload.py` identifies the primary cook section as the one with `prep_method=''` AND `cook_method=recipe.cooking_method`. If you set `prep_method='simmer'` (or any active method) on every section, no section qualifies as primary, and `cook_minutes` falls back to the value already in `recipes.csv`. Always update `cook_minutes` in `recipes.csv` at the same time you change `prep_method` values — never rely on `boil_stages` alone to carry the top-bar time after `prep_method` changes.
...
| SIDE_027 | Deviled Eggs | (none) | Rule D ✅ — no canonical; egg_cooked_hardboiled(1129) 300g+mayonnaise(4025) 41.4g+mustard_yellow(2046) 5.0g+apple_cider_vinegar(2048) 5.0g+salt_table(2047) 1.5g+black_pepper_ground(2030) 0.275g+paprika(2028) 1.15g; raw yfw=1.0 → 354.3g; 6 servings × 2 halves; 212.9 kcal·10.87P·17.82F·1.34C per 100g; dietary_category=veggie; committed `2436cac` |
...
- **⚠️ "Prep: X | unheated | all ingredients" on a composite recipe does NOT mean merge into one section.** When the user's prep stage format uses "all ingredients" for a recipe that contains component-refs (`@CHILD_ID` rows in recipe_ingredients.csv), interpret the spec as "update prep_method/locks on the existing section structure" — not as "flatten everything into one section." Each component-ref must remain in its own pure section so it collapses in the recipe display. Only truly leaf-ingredient sections may be merged or renamed. (Mistake made during SAUCE_013 unlock, July 2026.)
- **⚠️ Mixed sections (component-ref + leaf ingredients in the same section) will NOT collapse.** Auto-collapse only fires when a section is a PURE component-ref (all items are `isDish: true`). Any section that mixes a `@CHILD_ID` row with leaf ingredients stays expanded and the stock/sauce name appears incorrectly concatenated with the leaf section label. Fix: split the section — one pure `{key}_ref` section for the component-ref only (`cm=raw`, `source_recipe=CHILD_ID`, label = child recipe name), and the original section for leaf ingredients. **This problem was found across 17 SOUP recipes (July 2026) and may exist in other categories (ENTR, SIDE, PASTA, SAND) wherever a component-ref ingredient shares a section with leaf ingredients.** Run the mixed-section audit before releasing any category: `python3 -c "import csv; from collections import defaultdict; [... groupby recipe_id|section, flag has_ref and has_leaf ...]"`.
...
- **⚠️ Every new `fill_class` added to `yield_calc.py` BINDING must be simultaneously added to `yieldCalc.ts` BINDING.** These two tables are a mirror pair — `yield_calc.py` drives the Python dev recipe pipeline; `yieldCalc.ts` (`src/lib/nutrition/yieldCalc.ts`) drives the TypeScript community recipe path. A fill class present in only one produces different nutrition results for the same recipe depending on whether it is viewed as a dev recipe or a community recipe. Calibration data (the binding coefficient) and the NDB pair citation must appear in both files' comments. Current fill classes requiring parity: `dense_fruit`, `strudel_fruit`, `thickened_fruit`, `mincemeat`, `moderate_fruit`, `syrup_custard`, `vegetable_custard`, `dairy_custard`, `starch_custard`, `cake_batter`, `pastry`, `crumb_crust`, `none`, `simmer_sauce`, `pan_grilled_batter`, `fried_meat`, `fried_potato`, `grilled_batter`, `baked_pork`, `braised_beef`, `pan_grilled_chicken`, `fried_chicken`. (Discovered July 2026 when `baked_pork` was added to Python but not TypeScript.)
- **✅ RESOLVED (July 2026) — `fill_class_hint` column added to `DataCentralCombo` for community recipe accuracy.** `DataCentralCombo.fill_class_hint TEXT` column added to local comboo.db and Turso SR28. Populated for 12 protein NDBs across 4 fill classes (pan_grilled_chicken, baked_pork, braised_beef, fried_meat). `buildRecipeCommunityV3.ts` checks `fillClassHint` against a cook-method map before calling `inferFillingClass()`. The highest-protein ingredient's hint wins when multiple hints are present. New NDBs can be added via `UPDATE DataCentralCombo SET fill_class_hint='<class>' WHERE NDB_No=<ndb>` in both local comboo.db and Turso SR28.
  **Until this is implemented:** community recipes with whole-muscle proteins in baked/braised sections compute yfw=1.0. Dev recipes are unaffected (they have explicit `fill_class` in `recipe_sections.csv`). Community users editing a dev recipe template inherit the correct `fill_class` from `sections_json` — only scratch-built community recipes are broken.
...
- **⚠️ When restructuring a recipe from single-section to multi-section, always explicitly clear `cooking_method` in `recipes.csv` if the new design has a blank top bar.** `upload.py` writes `recipes.csv.cooking_method` verbatim to Turso every time — it never infers a blank from section structure. A recipe originally authored as single-section baked (`cooking_method='baked'`) that is later split into multiple sections where each section owns its own prep display will silently keep the stale `'baked'` in Turso unless `recipes.csv` is updated. The "changed cols" report only shows changes — if the value was already `'baked'` in both the CSV and Turso, no change is reported and the wrong top bar persists. Fix: clear `cooking_method` in recipes.csv, then re-upload. (Discovered July 2026, SAND_032 restructuring.)
...
- **Builtin recipes (SWEET/BKFST/SAND and all other dev prefixes)**: The recipe display page reads section data from **Turso** as its primary source. `generated-levels.ts` (the bundle) is a static fallback used only when Turso is unavailable. The edit form (moderator) also reads from Turso via the v3-build API. `formatSectionHeader` in `RecipeBook.svelte` calls `selectedLevel.sections ?? selectedLevel.nutritionJson?.sections` — `selectedLevel.sections` is populated from the bundle when the bundle is the source, and from Turso's response when Turso is the source. **Both the bundle and Turso must be kept in sync**: edit `recipe_sections.csv` → run `generate_bundle.py` → `upload.py --commit` (both in the same push). If only `upload.py` is run without `generate_bundle.py`, the bundle (fallback) will be stale and show wrong section headers if Turso becomes unavailable.
- **🚨 BUG — bundle is incorrectly participating in section display (July 2026).** The architectural intent is that the bundle is ONLY a quarantinable fallback: if the bundle were removed, the display should still work entirely from Turso. This required two separate fixes to make true:
  - **Fix 1 — `formatSectionHeader` key mismatch** (July 2026): `formatSectionHeader` was looking for `s.key` (camelCase) but Turso sections use `section_key` (snake_case). Fixed: now checks `(s.section_key ?? s.key) === sectionKey`.
  - **Fix 2 — `level-overrides.ts` sections promotion** (July 2026): Even with Fix 1 in place, `selectedLevel.sections` always came from the bundle (stale `prepMethod`) because the merge in `getLevelsWithOverrides()` never overrode `sections`. `selectedLevel.nutritionJson?.sections` had the up-to-date Turso data, but `selectedLevel.sections ?? nutritionJson.sections` always short-circuited on the bundle. Fixed: `level-overrides.ts` now promotes `override.nutritionJson?.sections` to `selectedLevel.sections` when available. **Both fixes are required** — Fix 1 alone does nothing if bundle sections are still served; Fix 2 alone does nothing if `formatSectionHeader` can't find the section by key.
  - **Why name was correct but cook method was not**: The section label (`section_label`) hasn't changed since recipe creation. `prep_method` changed when the recipe was unlocked (from `'raw'` to `'fried'`). Bundle had the old `prepMethod: 'raw'`; Turso had the new `prep_method: 'fried'`. After Fix 2, `selectedLevel.sections` is the Turso data and `prep_method: 'fried'` is read correctly.
  - **Result**: `upload.py --commit` alone is now sufficient to update section header display without running `generate_bundle.py`.
...
- **fill_class requirement applies to ALL stovetop sections — not just fried.** The same `elif s.filling_class and (s.cook_stages or s.boil_stages)` condition governs simmer/boil/sub-simmer/braise sections too. Without a `fill_class`, any stovetop section defaults to `yfw=1.0` (no evaporation) even with `boil_stages` set. Use `fill_class='simmer_sauce'` for sauce/gravy/roux sections that reduce on a stovetop. Use `fill_class='pan_grilled_batter'` for roux sections cooked with `pm='pan grilled'`. Omitting `fill_class` from a stovetop section silently produces `yfw=1.0` — this is the most common cause of incorrect water-loss physics in sauce recipes.
- **Lock removal applies to BOTH `recipe_sections.csv` AND `recipes.csv`.** `yield_factor_water`, `yield_factor_fat`, `yield_factor_protein`, `yield_factor_carbohydrate`, `yield_factor_other` columns exist at the recipe level in `recipes.csv` (cols 14–18) as well as at the section level. For recipes WITHOUT sections (single-section legacy path), the recipe-level locks ARE used to compute nutrition and must be cleared. For recipes WITH sections (multi-section path), `_build_recipe_multi` ignores recipe-level yield factors entirely — section-level physics takes precedence. However, clear both levels anyway for cleanliness and to avoid confusion in audit tooling.
