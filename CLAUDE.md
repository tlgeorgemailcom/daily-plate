...
- **Ingredient prep methods belong in `qty_display`, not in instructions**: When an ingredient requires a specific preparation (mincing, dicing, slicing, grating, etc.), embed it directly in the `qty_display` field — e.g. `"4 cloves, minced"`, `"1 medium, finely diced"`, `"3 tbsp, freshly grated"`, `"2 lb, cut into 1½-inch pieces"`. Do NOT repeat these prep notes in the instruction steps. Instructions that already have prep in `qty_display` do not need to re-specify it — e.g. "Add the garlic" is correct when `qty_display` already says `"4 cloves, minced"`. Established June 2026 during garlic audit of ENTR_052–069.
