<script lang="ts">
  ...
  function formatPerServingLabel(level: Level) {
    ...
    const servingsStr = level.servings?.trim().replace(/\s*\((serves|makes)[^)]*\)/gi, '');
    if (servingsStr) {
  ...
  let collabInitialData = $derived((): Partial<RecipeFormData> => {
    ...
    if (collabDraft) {
      ...
      return {
        ...
        linkMode: d.linkMode ? (d.linkMode === 'dish' && levelDishLink ? 'mixed' : d.linkMode) : (selectedLevel.linkType === 'dish' && levelDishLink ? 'mixed' : selectedLevel.linkType === 'ingredient' || selectedLevel.linkType === 'dish' || selectedLevel.linkType === 'mixed' ? selectedLevel.linkType : (levelDishLink ? 'mixed' : 'ingredient')),
