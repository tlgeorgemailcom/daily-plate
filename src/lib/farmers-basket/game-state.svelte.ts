...
import { LEVELS } from './generated-levels';
...
// LEVELS is imported from generated-levels.ts (auto-generated from recipes.csv)
export { LEVELS } from './generated-levels';
// Re-export for backwards compatibility — remove the line below and replace with:
// (nothing — the export above covers it)
const _LEVELS_PLACEHOLDER: never[] = [
  {
    id: '1-1',
    name: 'Simple Salad',
    category: 'Salads',
...
export function createGameState() {
  ...
  // Mark a level completed without running gameplay.
  function completeLevel(levelId: string) {
    const level = allLevels.find(l => l.id === levelId);
    if (!level) return;

    completedLevels = new Set([...completedLevels, levelId]);
    saveCompletedLevels(completedLevels);
  }
