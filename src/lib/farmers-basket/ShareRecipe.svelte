<script lang="ts">
  ...
  function buildPayload(data: RecipeFormData, imageUrl: string | null): Record<string, unknown> {
    ...
    return {
      ...
      cookingMethod: data.cookingMethod || 'Bake',
      ...(data.fillClass ? { fillClass: data.fillClass } : {}),
      ...(data.cook2Method ? { cook2Method: data.cook2Method } : {}),
      ...(typeof data.cook2Minutes === 'number' ? { cook2Minutes: data.cook2Minutes } : {}),
      ...(typeof data.cook2TempF === 'number' ? { cook2TempF: data.cook2TempF } : {}),
      ...(data.cook2FillClass ? { cook2FillClass: data.cook2FillClass } : {}),
      ...(data.cook3Method ? { cook3Method: data.cook3Method } : {}),
      ...(typeof data.cook3Minutes === 'number' ? { cook3Minutes: data.cook3Minutes } : {}),
      ...(typeof data.cook3TempF === 'number' ? { cook3TempF: data.cook3TempF } : {}),
      ...(data.cook3FillClass ? { cook3FillClass: data.cook3FillClass } : {}),
        ...
        {#if showNutrientInfo}
          <div class="info-panel">
            ...
            <div class="info-panel-body">
              ...
              <p><strong>Link ingredients exactly as your recipe writes them</strong> — raw, dry, or uncooked. Developer recipes already use cooked USDA dish values or recipe-level yield math when no whole-dish lab value exists. Community recipe links are saved on the basis you enter, so choose the primary cooking method that best matches the finished dish.</p>
        ...
        {#if showLinkingMethodsInfo}
          <div class="info-panel">
            ...
            <div class="info-panel-body">
              ...
              <p>There are three ways to link your recipe to the nutrition database. In all three methods, <strong>link ingredients exactly as your recipe writes them</strong> — raw, dry, or uncooked. Developer recipes use either cooked USDA dish values or recipe-level yield handling; for community recipes, pick the primary cooking method that best matches the finished dish so the saved links stay consistent.</p>
        ...
        {#if showRankingsInfo}
          <div class="info-panel">
            ...
            <div class="info-panel-body">
              ...
              <p>Recipes in Breakfast, Soups &amp; Stews, Sandwiches &amp; Burgers, Salads, Pasta &amp; Pizza, Entrees &amp; Main Courses, Sides, Sweets &amp; Desserts, Beverages, and Sauces &amp; Condiments are organised into two tiers. Within each tier, recipes shuffle daily so every recipe gets seen. The highest-ranked recipe in each tier is always pinned at the top.</p>
