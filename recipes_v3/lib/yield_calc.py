...
BINDING: dict[str, float] = {
    ...
    "sauteed_aromatic":  0.127, # moderate-heat sauteed aromatics (garlic, onion, shallot, sofrito, sliced mushrooms)
                                  # 200°F×2min → yfw≈0.956; 200°F×9min → yfw≈0.892.
                                  # Not for larger vegetable pieces like broccoli florets or green beans; calibrate those separately.
    ...
    "fried_breaded_shrimp": 0.749, # fried breaded shrimp — TodayPage cornmeal test: 212°F×3min → yfw≈0.62
                                  # Shrimp uniquely continued dripping moisture while resting over oil; protein purge drives this class.
                                  # Use for dry-breaded/flour/cornmeal shrimp, not wet-batter tempura or beer batter.
    "fried_breaded_chicken_tender": 0.342, # fried breaded chicken tender — TodayPage cornmeal test: 212°F×3min → yfw≈0.827
                                  # Calibrated from boneless breast tender strip; do not reuse for thighs without a separate naked-weight test.
    "fried_breaded_fish_fillet": 0.349, # fried breaded fish fillet — TodayPage cornmeal drum-fillet test: 212°F×3min → yfw≈0.823
                                  # Use for dry-breaded/cornmeal fish fillets; keep true wet-batter fish as a separate future class.
    ...
    "casserole_baked":       0.550, # baked casserole with cream sauce (green bean casserole etc.) — SIDE_034: 350°F×25+5min → yfw=0.88
                                  # Calibrated to SIDE_034 original locked yfw=0.88.
    "roasted_vegetable":     1.175, # high-heat roasted vegetables (asparagus, Brussels sprouts, mushrooms) — SIDE_037/038/039: 425°F×12min → yfw=0.85
    ...
    "pizza_cheese_topping":  0.450, # pizza cheese/herb toppings baked at high heat — PASTA_003/004: 450°F×20min → yfw=0.90
                                  # Calibrated to PASTA_003/004 original locked yfw=0.90.
    ...
    "sub_simmered_beef":   0.400, # beef simmered in cooking liquid — ENTR_010: 180°F×165min → yfw=0.60
                                  # Use when cooking liquid is not counted in the finished serving grams.
